...
## Bite 3 RECEIPT — viability probe result (2026-08-01)

**Headline: no break observed through N=1,700 on the faithful synthetic shape.** Every rung completed extract → untargeted `weave` → standalone `generate` with coherent counts and byte-stable same-timestamp regeneration. This is the first end-to-end evidence that the supported sequence works at Stagecraft's cardinality on the no-ancestor-Knop topology. Run from a clean `main` worktree; no repository file modified.

| N | weave wall / max RSS | generate wall / max RSS | woven | pending after | pages (term subtree) | same-timestamp drift |
|---:|---|---|---:|---:|---:|---|
| 3 | 0.40s / 163 MiB | 0.30s / 158 MiB | 3 | 0 | 12 | none (43 files) |
| 50 | 1.30s / 244 MiB | 0.90s / 226 MiB | 50 | 0 | 200 | none (372 files) |
| 200 | 7.00s / 467 MiB | 4.00s / 482 MiB | 200 | 0 | 800 | none (1,422 files) |
| 500 | 32.23s / 1.45 GiB | 15.61s / 1.47 GiB | 500 | 0 | 2,000 | none (3,522 files) |
| 1,000 | 1m55.60s / 2.79 GiB | 50.44s / 2.76 GiB | 1,000 | 0 | 4,000 | none (7,022 files) |
| 1,700 | 5m09.98s / **3.79 GiB** | 2m29.63s / **3.74 GiB** | 1,700 | 0 | 6,800 | none (11,922 files) |

Per rung the pre-weave census was exactly N pending designators all bound to `catalog/source`; `catalog/source/_knop` existed and `catalog/_knop` never appeared in filesystem or MeshInventory; post-weave pending was zero; every term and Knop carried its canonical page claim; first/middle/last pages contained source-derived facts; source facts and non-HTML artifact hashes were unchanged; and standalone generate created 0 files and updated 0 pages.

### The load-bearing caveat: it passes with almost no headroom

N=1,700 weave peaked at **3.79 GiB against V8's ~4.09 GiB default old-space ceiling** — the same ceiling that produced the original srd OOM. It passes; it barely passes. Timing shows recursive planning (135.3s) and page generation (129.6s) dominate, while RDF validation (128.5ms) and writes (118.1ms) are noise. Extracted candidates never enter the batch path (`payloadBatchCandidates=0` — #33 gates on all-`firstPayloadWeave`, and these classify as `firstExtractedKnopWeave`), so they run the per-candidate recursive loop: candidate-cache hits reached 1,445,850 at N=1,700, the exact triangular per-candidate pattern. **This result strengthens rather than retires the batching / incremental-MeshInventory follow-up.**

### sflo `gh-pages` arm: FAILED, for an unrelated and more interesting reason

Materialized ref `a7e7626` into `/tmp` (checkout untouched; census confirmed 7,234 files / 379 Knops, and both `config/_knop` and `ontology/_knop` present, so this arm never could reproduce the defect shape). Standalone `generate` exited **1 in 0.20s**, before render or write:

> `Could not resolve the current extracted source target for config/allowedRemoteOrigin.` — phase `generate.collectGeneratedPageFiles.loadDesignatorContexts`

Cause: **the corpus is on legacy vocabulary.** It binds with `sflo:hasTargetArtifact` / `sflo:hasRequestedTargetState`; current main resolves `sflo:targetArtifact` / `sflo:targetHistoricalState`. The failed run mutated nothing (7,234-file and 2,659-Turtle manifests byte-identical). So the arm proved neither correctness nor its negation — it surfaced that **our own flagship published corpus can no longer be regenerated by current code**, which is a migration decision, not an extractor defect. Boarded as its own court card.

### What this does and does not authorize

It does NOT authorize telling Stagecraft their workaround can retire. The synthetic shape is faithful but synthetic; a real-corpus replay or their own confirmation still gates that, exactly as [[wd.consumer-feedback.0.5.1.reply]] §2 promises ("we'll say so explicitly"). What it does authorize: stating internally that the sequence is no longer *known-broken* at their cardinality, and prioritizing the scale work on measured curves instead of guesses.

Residual: only current-only history was measured; no versioned comparison exists. The generator exposes no `--generated-at` (fixed timestamps were applied to the measured weave/generate operations only).
