...
- [d] Finish the branch-published first-release replay rewrite after `integrate` supports working-only source bindings without repository ref/commit/digest evidence; once the initial bindings are correct, first-release should weave from the already-integrated working locators rather than updating source bindings.
- [d] Split CI-action idempotency coverage into [[wa.completed.2026.2026-05-21_0820-ci-idempotency-tests]]. Keep `remove-prepare` focused on removing the bespoke command and decomposing its behavior into generic operations.

The branch-published conformance manifests no longer use `prepare gh-pages` for publication bootstrap or initial source integrations. The remaining first-release manifest is different: it should not re-run `integrate`, `payload.update`, or a source-binding update when the source locator is stable. It should materialize the desired source checkout and weave/version from the already-integrated working locators. The implementation gap is that first-time `integrate` still needs a clean working-only source-binding CLI path that does not force repository ref/commit/digest evidence onto floating working sources.
