---
id: weave-and-accord-improve-z2p11h
title: 'weave and accord improvements for stagecraft'
desc: ''
created: 1783403540132
updated: 1783659376024
...
- [[ac.task.2026.2026-07-04-scenario-runner]] — `accord check-scenario` with per-step evidence grouping (product bets 2 + 6). Important find: the JSON-LD scenario-index loader and the `ScenarioIndex`/`ScenarioStep`/`StateLane` vocabulary **already exist** in `src/scenario/` and the ontology — only the CLI runner is missing, so this is smaller than it looked.
- [[ac.task.2026.2026-07-04-json-assertions]] — native JSON assertions with `notExists` as a first-class absence proof (bet 1 plus the JSON half of bet 5). First slice: `exists`/`notExists`/`equals`/`count`; the "iff" conditional shape is explicitly deferred.
- [[ac.task.2026.2026-07-04-draft-manifest]] — conservative `accord draft-manifest --from --to` scaffolding from `git diff --name-status` (bet 3), with deterministic/idempotent output as a hard requirement.

**On Testing Vocab: agreed, out of scope.** The HttpInteractionCase/OpenAPI direction is a genuinely different product bet (runner-neutral REST conformance), not something Stagecraft needs this round. I added a **Task Ownership section** to the top of [[ac.product-ideas.runner-neutral-test-spec]] mapping every idea to its owning task and explicitly parking Testing Vocab, profile packs, immutability packs, and drift checks (drift is noted as a likely second slice inside the scenario-runner task so it doesn't get lost).

**Sequencing** is recorded in the [[wa.task.2026.2026-06-30_1108-stagecraft-driven-semantic-flow-requirements]], which stays open as the Stagecraft-side requirements driver.
...
The blocker lives in [[wa.task.2026.2026-07-03_1332-stagecraft-weave-planner-generalization]], which already has an excellent captured repro (test-inn-ambush, `a.11` → `a.12`, three designators wanting `_s0003`/`_s0004`). I verified the diagnosis against the planner source, and it's structural:
...
**Mechanics: yes.** [[wu.cli-reference.weave]] and the `version` reference both explain batching semantics (canonical order, merged progression, no-op reruns) and snapshot verification — including that a changed working payload refuses the whole batch before anything is written, with a diagnostic naming the file, and that post-capture changes are ignored by design.
...
- Updated the four references in the [[wa.task.2026.2026-06-30_1108-stagecraft-driven-semantic-flow-requirements]] as well. I left the `wa.conv.*` logs alone — they're historical records, and their references are plain text, not resolvable wikilinks.
...
**[[release-notes.v0.3.0]]** is fully drafted in the house style — no TODOs left (the workflow strips frontmatter and fails on an empty body, so it's release-workflow-ready). Content decisions worth knowing about:
...
# claude-opus-4.8_2026-07-06_2253_12

Good — you've talked it to the simple answer, and it is the right one. Let me clear up my sloppy phrase first, because your clarifying question exposes a real distinction I blurred.

## What I mis-said

"Realize an ordered list from supplied working files" conflated two different multiplicities:

- **Multiple DigitalArtifacts, one new state each** — this is the v0.3.0 batch (the three inn-ambush designators versioned together in one plan). It's your "multiple DigitalArtifacts woven together" case.
- **One DigitalArtifact, multiple pending states** — memory advanced s0042→s0043→s0044 and all three need realizing. This is your "single artifact, multiple states" case, and **the batch does not do it.** It only arises if you're flushing an in-memory sequence that advanced several times between flushes.

So the honest statement is: the batch versions *multiple artifacts, one state each*. And that's exactly the shape of a save-game snapshot — several artifacts changed, version them together atomically. The "one artifact, many pending states" shape is the thing that would have needed new machinery, and it's precisely the thing a filesystem-backed mesh makes disappear: if the app writes a working file at save time, each save is one new state per artifact. You never accumulate a multi-state backlog to flush.

## So your conclusion is correct: nothing needs to change

Filesystem-backed mesh per campaign + app writes working files at save-game time + call `weave version` (or the batch when several artifacts changed) = fully covered by what v0.3.0 already shipped. Later-payload generalization handles `_s0003+` with current-only support; the batch handles multi-artifact atomicity; snapshot verification guards the app writing working files. Literally zero new Weave surface. It's the simplest thing that works, and "keep a backing filesystem mesh on disk" is the call I'd make too — it also means you never take on the weave-as-library packaging surface, which (per the accord JSR bug you're patching) is not free.

## What survives as deferred, not dead

- **The "bytes + IRI + optional metadata → weave version" API** is real and worth wanting — it's a thin content-source-plus-identity front door onto the same planner, saving the working-file round-trip. But it's the thing that drags in the library-packaging + resource-loading audit, so deferring it until an app actually wants to skip the disk write is right. When you do build it, fork-answer (a) is what makes it clean: caller supplies the state IRI so it can author references (like the manifest) before the flush.
- **The checkpoint manifest** stays a good idea "anyhow" as you said — but it's Stagecraft-authored payload RDF, not a Weave feature, so it gates nothing here and lands whenever you want the per-checkpoint audit trail. Granularity is the app's call, as you noted.

The one thing I'd flag as worth *not* losing: this thread plus the two SFF reports (`sf.report...codex` and `...chatgp`) contain a genuine conclusion — "API-only if ever, ontology already sufficient, filesystem-mesh makes it moot for now" — and right now that lives only in the conversation. Want me to drop a short decision-capture into the umbrella note (the conclusion, the two deferred ideas, and pointers to the two reports) so the next person who wonders "should Weave support in-memory artifacts?" finds the answer instead of re-deriving it? A few lines, not a task note — since the decision is "defer," not "do."

# djradon_2026-07-06_2259_02

Could you write the bytes + IRI + optional metadata into sf.product-ideas.api.md and link it from the roadmap?

# claude-opus-4.8_2026-07-06_2259_06

Let me look at the target file and find the roadmap it should link from.

# claude-opus-4.8_2026-07-06_2259_10

Empty except frontmatter. Let me find the roadmap and check the vault's conventions.

# claude-opus-4.8_2026-07-06_2259_49

`sf.todo.md` is the roadmap (Immediately/Eventually), and `sf.api.md` is the API map. Writing the idea into the product-ideas note in the vault's voice, then linking from the roadmap's Eventually section.

# claude-opus-4.8_2026-07-06_2300_14

Let me finish the roadmap link, then answer your question.

# claude-opus-4.8_2026-07-06_2300_50

Done — the idea is in [sf.product-ideas.api.md](dependencies/github.com/semantic-flow/semantic-flow-framework/notes/sf.product-ideas.api.md) and linked from the Eventually section of [sf.todo.md](dependencies/github.com/semantic-flow/semantic-flow-framework/notes/sf.todo.md).

## On "Later-payload generalization handles `_s0003+` with current-only support"

That was a compressed way of naming what the v0.3.0 planner epic actually fixed. It's three separate pieces:

**"Later-payload generalization"** — the planner change from weave `6e4068f`/`e7029ab`. Before it, the payload planner could only advance an artifact to its *second* state. `resolveFirstPayloadVersionLayout` and `resolveSecondPayloadVersionLayout` were the only layout resolvers, ~13 render templates hardcoded `sflo:nextStateOrdinal "2"`, and there was a `...ForSecondPayloadWeave` shape assertion. The generalization replaced that with a read model that derives the next state from RDF facts (`sflo:latestHistoricalState` + `sflo:nextStateOrdinal`), so any ordinal works.

**"`_s0003+`"** — third historical state and beyond. That's the concrete thing that was broken before: an artifact already sitting at `_s0003` couldn't advance, because the old code only recognized the second-state shape and threw "the current local weave slice only supports the settled second payload weave shape." After v0.3.0, `_s0003 → _s0004 → …` advances normally. This is what makes it relevant to your save-game flow: an artifact that's been versioned many times is well past `_s0002`.

**"current-only support"** — the *support* artifacts around a payload (its KnopInventory, KnopMetadata, page definition) can be kept **current-only** instead of **versioned**: they have just a live current file, no `_history*/_s*` tree of their own, while the *payload* still keeps a full versioned history. The Stagecraft temporal rung was shaped exactly this way, and it was part of why the old planner choked — `detectPendingWeaveSlice` wouldn't even classify a later-payload weave when the support artifacts had no histories of their own, unless you fed it hints. The generalization made that classification work without hints.

Put together: **an application's versioned payloads can be at their 3rd, 4th, Nth state with lean current-only support artifacts around them, and `weave version` will advance them correctly** — which before v0.3.0 it could not. That's why I cited it as evidence the save-game flow needs nothing new: a long-running campaign produces exactly that shape (many payload versions, lean support), and it's now handled.

# djradon_2026-07-06_2348_47

What do you think of the footgun we identified in stagecraft: "mesh create resolves relative paths from the current process cwd, not the workspace path, so --mesh-root . was rejected in scratch. That is a small CLI footgun; I’m retrying with absolute mesh root."

# claude-opus-4.8_2026-07-06_2349_00

Good catch to interrogate — "rejected" is a strong word, and whether this is a footgun or correct-behavior-with-a-bad-message depends on *why* it rejected. Let me look at the actual resolution code before giving you an opinion.

# claude-opus-4.8_2026-07-06_2351_12

Your diagnosis is exactly right, and it's a sharper footgun than "small" suggests. Here's the precise mechanism, then what I'd change.

## What actually happens

`mesh create` is the *only* command with two separate path inputs — `--workspace` (default `.`) and `--mesh-root` — because at create time there's no existing mesh to derive the workspace from. Every other command (`weave`, `version`, `generate`, …) takes just `--mesh-root`, resolves it from cwd, and treats it *as* the mesh, deriving the workspace by walking up. So `mesh create` already has special path semantics.

The trap is in [normalizeCliMeshRoot](src/cli/run.ts#L1368-L1390):

```
const absoluteMeshRoot = resolve(meshRoot);          // resolve(".") = process cwd
const relation = relative(workspaceRoot, absoluteMeshRoot);
if (relation.startsWith("../") ...) throw "mesh root must stay inside the workspace root: ."
```

So `--mesh-root .` resolves against **process cwd**, but the constraint checks it against **`--workspace`**. When cwd ≠ workspace (you passed `--workspace <scratch>` while your shell sat in the repo), `.` resolves to the repo, `relative(scratch, repo)` starts with `../`, and it throws — echoing back the literal `.` you typed, which is why the message reads as nonsense ("but `.` *is* inside!"). You and the code disagree about what `.` means.

Three things make it a genuine footgun, not user error:

1. **The most intuitive invocation of the most common topology is the one that breaks.** `--workspace X --mesh-root .` reads as "mesh is the whole workspace" (whole-repo topology) — but `.` binds to cwd, not workspace.
2. **The error message hides the actual anchoring.** It shows your raw input `.` instead of the two resolved absolute paths, so nothing points you at the cwd-vs-workspace mismatch.
3. **There's a hidden correct path you'd never guess:** *omitting* `--mesh-root` entirely does exactly what you wanted — [line 1372-1373](src/cli/run.ts#L1372-L1373), `meshRoot === undefined → return "."` means workspace-root. So "mesh = workspace" works by leaving the flag off, and breaks when you spell it out. That's backwards from every user's instinct.

Your absolute-path workaround is correct and for the right reason: an absolute `--mesh-root` doesn't depend on cwd, so the workspace check sees a clean subpath.

## What I'd change

Two levels, and I'd do both:

- **Must-fix (safe): make the error self-explanatory.** Print both resolved absolute paths — "mesh root `<abs>` is outside workspace root `<abs>`" — and, since omitting the flag is the fix for the common case, add "to place the mesh at the workspace root, omit `--mesh-root`." That alone converts the trap into a teaching moment with zero behavior change.
- **Judgment call (ergonomic): anchor relative `--mesh-root` to the workspace instead of cwd.** Then `--mesh-root .` = workspace root and `--mesh-root sidecar` = `<workspace>/sidecar`, matching both the omit-default and the natural reading, and the "must stay inside workspace" check becomes coherent with its own input. The cost is breaking the Unix `.`=cwd convention for this one flag — which is exactly the convention that bit you here, and `mesh create` is a one-time bootstrap with an easy absolute-path escape, so blast radius is tiny. The help text at [line 1177-1179](src/cli/run.ts#L1177-L1179) would flip accordingly.

I lean toward doing both — the message fix is non-negotiable, and the anchoring change is where I'd land but it's your call since it trades a Unix convention for ergonomics. Want me to make the change in weave? It's a contained edit in `normalizeCliMeshRoot` plus the help text and a couple of CLI tests; I'd hold it for a v0.3.1 since v0.3.0 notes are already frozen.

# djradon_2026-07-08_1625_56

OK, let's make it sefl-explanatory. I have another "friendlier error" request from stagecraft. `weave:validate` fails: "Cannot auto-version payload artifact world/states/inn-ambush-plan-b-state because current payload history …/main uses NAMED historical state …/main/s0002. Provide stateSegment on the target, or explicitly request ordinal fallback.... Optional courtesy follow-up for the weave dev arc (roadmap): a friendlier error suggesting the current head name."

# claude-opus-4.8_2026-07-08_1626_40

Two things here: implement the self-explanatory `mesh create` error now, and capture the `weave validate` NAMED-state error as a roadmap item. Let me find the test surface for the first and locate the second error's source.

# claude-opus-4.8_2026-07-08_1627_10

Both errors located. Let me read the existing mesh-create test to match its style before editing.

# claude-opus-4.8_2026-07-08_1627_28

This existing test *is* the footgun — `--workspace docs --mesh-root .` from cwd `<tmp>`, so `.` resolves to the parent of `docs` and gets rejected. Perfect anchor for the fix. Making the three edits:

# claude-opus-4.8_2026-07-08_1627_54

Now update the test assertion to match the self-explanatory message:

# claude-opus-4.8_2026-07-08_1628_01

Now the roadmap item for the NAMED-state error, then I'll run the gates. Let me place it in the weave todo's P2 (developer-experience) section.

# claude-opus-4.8_2026-07-08_1628_25

Passing. Let me capture the actual rendered message so you can see what it now reads like:

# claude-opus-4.8_2026-07-08_1628_44

Both done.

## 1. `mesh create` error — now self-explanatory (implemented)

The message you'd have hit in scratch now reads:

> mesh root "." resolves to `/tmp/…`, which is outside the workspace root `/tmp/…/docs`. Relative --mesh-root values are resolved from the current directory, not the workspace. To place the mesh at the workspace root, omit --mesh-root; otherwise pass a path inside the workspace.

That surfaces all three things the old message hid: what `.` actually resolved to, what the workspace actually is, and the two ways out (the key one being *omit the flag* — which is the hidden correct path for "mesh = workspace"). Changes:

- [payload the error in normalizeCliMeshRoot](src/cli/run.ts#L1384-L1390) — names both resolved absolute paths + the cwd-anchoring explanation + remedies.
- [help text](src/cli/run.ts#L1177-L1179) — now "Mesh root path inside the workspace… omit to place the mesh at the workspace root."
- Updated the existing rejection test (which already exercised this exact footgun) to assert the self-explanatory phrasing.

I kept cwd-anchoring rather than switching to workspace-anchoring, per your "make it self-explanatory" — no behavior change, just a message that teaches. fmt + typecheck clean, all 4 mesh-create e2e tests pass. I'll fold this into v0.3.1 alongside the accord JSR fix rather than touching frozen v0.3.0 notes.

## 2. Named-head auto-version error — captured on the roadmap

Added to [wd.todo.md](documentation/notes/wd.todo.md) P2. The mechanism, for the record: auto-version needs the current ordinal to compute the next one, but your head is `…/main/s0002` — and `s0002` has no leading underscore, so `parseOptionalStateOrdinalFromPath` treats it as a *named* state, not a `_sNNNN` default. It can't infer the next ordinal, so it refuses. The current message's only concrete suggestion is `stateSegment=_s0001`, which resets a named `sNNNN` history to the ordinal scheme — jarring. The roadmap item is to **suggest the incremented head name** when the head ends in an integer (`s0002` → suggest `stateSegment=s0003`), offering continuation in the history's own naming, with the generic message as fallback when no numeric suffix is inferable.

I scoped it as roadmap rather than doing it now because — unlike the mesh-create message — it's real logic with edge cases (numeric-suffix inference on arbitrary segment names), not a message rewrite. That said, it's a small, self-contained slice and pairs naturally with the mesh-create work; if you'd like it in the same v0.3.1 diagnostics-quality batch, say the word and I'll implement it now.