...
Status 2026-07-05: Accord steps 1 and 2 have landed (parser-backed ASK profile; standalone `accord validate`). The first Weave-general blocker predicted by this note is recorded in [[wa.completed.2026.2026-07-03_1332-stagecraft-weave-planner-generalization]]: the settled second-payload weave shape assertion blocked later-ordinal payload advancement for Stagecraft's temporal rung. That completed Weave-side track ran parallel to Accord steps 3-5 and took priority over them when effort was serial because it gated the rung itself rather than ergonomics.
...
- [x] Identify the first Weave blocker that prevents Stagecraft from persisting or inspecting useful roleplaying data. DONE: the extracted-term lifecycle at ~1,700-term nested-source scale ([[wa.completed.2026.2026-07-21_1603-extractor-defect-pair]]) — first implementation bite in flight.
