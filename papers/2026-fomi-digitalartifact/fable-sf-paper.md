...
Even a well-run identifier acquires its meaning socially. A class IRI's sense may narrow when a revision tightens its definition; a property may drift as a community uses it in ways the original comment did not anticipate; an entire vocabulary's role may shift as its ecosystem changes. Occasionally an identifier's current sense becomes outright inconsistent with assertions made against an earlier state.
